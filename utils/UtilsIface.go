...
type HELOFeatures struct {
	...
	Xerror          bool
	Collections     bool
	Json            bool // previously Datatype
}

func (h *HELOFeatures) String() string {
	return fmt.Sprintf("Enabled features: Xattribute: %v CompressionType: %v Xerror: %v Collections: %v Json: %v",
		h.Xattribute, base.CompressionTypeStrings[h.CompressionType], h.Xerror, h.Collections, h.Json)
}
